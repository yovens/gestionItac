...
$affects = $pdo->query("SELECT a.*, u.prenom, u.nom, m.nom as matiere, c.nom as classe 
                        FROM affectations a 
                        LEFT JOIN users u ON a.prof_id=u.id 
                        LEFT JOIN matieres m ON a.matiere_id=m.id 
                        LEFT JOIN classes c ON a.classe_id=c.id")->fetchAll(PDO::FETCH_ASSOC);
?>
<!doctype html>
<html lang="fr">
<head>
...
<title>Affectations — Admin ITAC</title>
...
.form-box select, .form-box input{flex:1;padding:10px;border-radius:8px;border:none;outline:none;background:#2a2f42;color:#fff;}
...
<div class="dashboard-container">
  ...
  <main class="main-content">
    ...
    <div class="form-box">
      <form method="post">
        <select name="prof_id"><?php foreach($profs as $p):?><option value="<?=$p['id']?>"><?=$p['prenom'].' '.$p['nom']?></option><?php endforeach;?></select>
        <select name="matiere_id"><?php foreach($matieres as $m):?><option value="<?=$m['id']?>"><?=htmlspecialchars($m['nom'])?></option><?php endforeach;?></select>
        <select name="classe_id"><?php foreach($classes as $c):?><option value="<?=$c['id']?>"><?=htmlspecialchars($c['nom'])?></option><?php endforeach;?></select>
        <input name="session_nom" placeholder="Ex: 1ère session">
    ...
    <div class="table-container">
      <table>
        <thead>
          <tr><th>#</th><th>Prof</th><th>Matière</th><th>Classe</th><th>Session</th><th>Action</th></tr>
        </thead>
        <tbody>
